...
from casebook.core.clientmode import ModeError, remote_url, resolve, say   # 모드는 clientmode 가 정한다

FACTS_TOOLS = ("list_threads", "open_thread", "switch_thread", "resume")
HIDDEN_TOOLS = ("hook", "import_prior")   # 훅·백필 스크립트 전용 — 호스트 모델에게 보이지 않는다
IDEMPOTENT_TOOLS = ("add_evidence", "note_turn")
REQUEST_ID_META = "casebook/request_id"   # mcp_server.REQUEST_ID_META 와 같은 값 — 서버 모듈을 불러오지 않으려고 따로 둔다
...
def _readable_error(exc: BaseException) -> str:
    ...
    if "401" in raw or "unauthorized" in raw.lower():
        return say("서버가 로그인을 거절했다. 터미널에서 casebook-login 을 실행해 다시 로그인한다.",
                   "The server rejected the login. Run casebook-login in a terminal and sign in again.")
    if "403" in raw or "forbidden" in raw.lower():
        return say("서버가 이 계정의 사용을 거절했다. 운영자에게 이용 권한을 확인한다.",
                   "The server denied access for this account. Ask the operator to check your access.")
    if isinstance(exc, TimeoutError) or not raw:
        return say("서버가 제시간에 답하지 않았다. 연결 프로그램은 살아 있으며 다음 호출에서 다시 시도한다.",
                   "The server did not answer in time. The connector is still running and will retry on the next call.")
    return say(f"서버 연결에 실패했다: {raw}", f"Could not connect to the server: {raw}")
...
def host_schema(tool: Any) -> dict[str, Any]:
    ...
    props.pop("request_id", None)  # 재시도 열쇠는 프록시가 한 호출에 한 번 만든다.
    schema["properties"] = props
    if "required" in schema:
        schema["required"] = [r for r in schema["required"] if r not in ("facts", "client", "request_id")]
    return schema
...
def _oauth_ready() -> bool:
    ...
    try:
        from casebook.adapters.device_login import _path, load
        exists = _path().is_file()
        ready = bool(load())
        if exists and not ready:
            raise LoginRequired(say(
                "로그인 파일을 읽을 수 없다. 터미널에서 casebook-login 을 실행해 다시 로그인한다.",
                "The login file cannot be read. Run casebook-login in a terminal and sign in again."))
        return ready
    except LoginRequired:
        raise
    except Exception as exc:                    # noqa: BLE001 — 원인을 숨기지 않고 로그인 길을 남긴다
        raise LoginRequired(say(
            f"로그인 정보를 읽지 못했다: {exc}. 터미널에서 casebook-login 을 실행해 다시 로그인한다.",
            f"Could not read login information: {exc}. Run casebook-login in a terminal and sign in again.")) from None
...
class Proxy:
    ...
    async def own(self, *, task_status=anyio.TASK_STATUS_IGNORED) -> None:
        ...
        try:
            await self.connect()
        except Exception as exc:                 # 로그인·서버 장애가 stdio 프로세스까지 끄지 않게 한다
            self.last_error = _readable_error(exc)
        self._owner_tx = tx
        task_status.started()
        try:
            async with rx:
                async for reply_tx in rx:
                    ...
                    try:
                        ...
                        pass                     # 부탁한 쪽이 기한을 넘겨 먼저 떠났다 — 소유 태스크는 계속 산다
        finally:
    ...
    def host_tools(self) -> list[Any]:
        ...
        return [types.Tool(
            ...
            description=(self.last_error or say("서버 연결 상태를 확인하고 복구를 시도한다.",
                                                "Check the server connection and try to recover.")),
    ...
    async def _send(self, name: str, args: dict[str, Any], request_id: str | None = None) -> Any:
        ...
        with anyio.fail_after(TOOL_TIMEOUT):
            ...
            return await self.session.call_tool(name, args, meta={REQUEST_ID_META: request_id})
    ...
    async def call(self, name: str, args: dict[str, Any] | None, client: str | None = None) -> Any:
        ...
        if name == "connection_status":
            ...
            except Exception as exc:             # 프로세스는 살아 있고, 사람이 할 일을 결과로 돌려준다
                message = _readable_error(exc)
                self.last_error = message
                return types.CallToolResult(
                    content=[types.TextContent(type="text", text=f"casebook: {message}")], is_error=True)
            return types.CallToolResult(content=[types.TextContent(
                type="text", text=say("casebook 서버에 다시 연결됐다. 도구 목록을 새로 불러온다.",
                                      "Reconnected to the casebook server. Refresh the tool list."))])
        args = self.with_client(name, self.with_facts(name, args or {}), client)
        # 한 호출에 열쇠 하나 — 다시 보내도 같은 열쇠라 서버가 한 번만 적는다. 입력칸이 아니라 _meta 로 간다.
        request_id = uuid.uuid4().hex if name in IDEMPOTENT_TOOLS else None
        try:
            if self.session is None:
                await self._reconnect(None)
            return await self._send(name, args, request_id)
            ...
            try:
                ...
                self.last_error = message
                return types.CallToolResult(
                    content=[types.TextContent(type="text", text=f"casebook remote error: {message} (after reconnect)")],
                    is_error=True)
...
async def call_and_announce(proxy: Proxy, ctx: Any, name: str, args: dict[str, Any] | None) -> Any:
    """호출을 넘기고, 도구 없이 떠 있다가 이 호출로 살아났으면 호스트에 목록이 바뀌었다고 알린다.

    켤 때 못 붙으면 호스트가 받은 목록은 connection_status 하나뿐이다. 알리지 않으면 다시 붙어도 호스트는
    그 목록을 쥔 채 세션을 보낸다."""
    had_tools = bool(proxy.tools)
    result = await proxy.call(name, args, client=client_name(ctx))
    if not had_tools and proxy.tools:
        try:
            await ctx.session.send_tool_list_changed()
        except Exception:  # noqa: BLE001 — 알림을 못 보내도 호출의 답은 간다
            pass
    return result
...
def build_server(proxy: Proxy):
    ...
    async def on_call_tool(ctx, params):
        return await call_and_announce(proxy, ctx, params.name, params.arguments)
