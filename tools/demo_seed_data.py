...
TOPICS = [
    {
        "name": "결제 대행사 교체",
        "at": "2026-08-18 09:20",
        "summary": "수수료와 부분취소 때문에 결제 대행사를 페이레일로 옮긴다. 후보 비교(8/18) → 샌드박스 연동(8/20) → 실결제 전환 점검(9/8).",
        "conclusion": "페이레일로 옮겼고 실결제 전환만 남았다\n\n부분취소를 API 로 열어 주는 곳이 둘뿐이었고 정산이 D+2 라 페이레일로 정했다. 샌드박스는 결제·부분취소·웹훅까지 통과했다. 남은 것은 실결제 키로 한 건을 태우는 일이고, 그건 대표 승인이 있어야 한다.",
        "threads": ["C1", "C2", "C3"],
    },
    {
        "name": "주문·결제 장애",
...
영업 담당에게 8일에 물었고 아직 답이 없다. 한도가 우리 하루 취소액(평균 240만원, 최대 810만원)보다 낮으면 월말에 취소가 막힌다. 답이 오면 계약서 부속에 넣고 켠다.""",
            "by": "user"}},
